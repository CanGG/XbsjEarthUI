  ...
  <div class="xbsj-list" ref="container" @mousedown="startMove($event)" @mousemove="onMoving($event)" @mouseup="endMove($event)">
    ...
    <div class="xbsj-list-item">
      ...
      <div class="xbsj-item-btnbox" @click="dispatchManeuverShow=!dispatchManeuverShow">
        <div class="xbsj-item-btn disasterButton" :class="dispatchManeuverShow?'disasterButtonActive':''"></div>
        <span class="xbsj-item-name">{{lang.disaster}}</span>
      </div>
      <!-- 调度记录 -->
      <div class="xbsj-item-btnbox" @click="showDispatchRecord()">
        <div class="xbsj-item-btn dispatchRecordButton" :class="dispatchRecordShow?'dispatchRecordButtonActive':''"></div>
    ...
    <div class="xbsj-list-item xbsj-list-lastitem">
      ...
      <div class="xbsj-item-btnbox" @click="showVehicleVideo">
        <div class="xbsj-item-btn vehicleVideoButton" :class="vehicleVideoShow?'vehicleVideoButtonActive':''"></div>
  ...
  methods: {
    ...
    showVehiclePosition() {
      ...
      if (!this.vehiclePositionShow) {
        ...
        that.vehiclePositionWinObj = window.open(this.$root.$hyControls.basePath + "/plan_rescue_pc/plan/emergency_rescue.html", "_blank", "left=262,top=250,heigh=582,width=385,scrollbars=yes,resizable=1,modal=false,alwaysRaised=yes");
    ...
    showPotionQuery() {
      ...
      if (!this.potionQueryShow) {
        ...
        that.potionQueryWinObj = window.open("https://sugar.aipage.com/dashboard/15a4349eb5b43259ae1a84e8bf62e013", "_blank", "left=262,top=250,heigh=582,width=385,scrollbars=yes,resizable=1,modal=false,alwaysRaised=yes");
        var loop = setInterval(function () {
    ...
    showVehicleVideo() {
      ...
      if (!this.vehicleVideoShow) {
        ...
        that.vehicleVideoWinObj = window.open(this.$root.$hyControls.basePath + "/plan_rescue_pc/earthui_h5/pages/video_tracking.html", "_blank", "left=262,top=250,heigh=582,width=385,scrollbars=yes,resizable=1,modal=false,alwaysRaised=yes");
        var loop = setInterval(function () {
    ...
    showDispatchRecord() {
      ...
      if (!this.dispatchRecordShow) {
        ...
        that.dispatchRecordWinObj = window.open(this.$root.$hyControls.basePath + "/plan_rescue_pc/earthui_h5/pages/dispatch_record.html", "_blank", "left=262,top=250,heigh=582,width=385,scrollbars=yes,resizable=1,modal=false,alwaysRaised=yes");
        var loop = setInterval(function () {
